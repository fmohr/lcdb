...
def compute_curve_for_svc(
    openmlid: int,
    outer_seed: int,
    inner_seed: int,
    train_size: int,
    monotonic: bool,
    complexity_constant: float,
    gamma: float,
    logger=None,
):
    learner = sklearn.svm.SVC(kernel="rbf", C=complexity_constant, gamma=gamma)
    return compute_result(
        openmlid, outer_seed, inner_seed, train_size, monotonic, learner, logger
    )
...
def compute_result(
    ...
    X_train, X_valid, X_test, y_train, y_valid, y_test = get_splits_for_anchor(
        X, y, train_size, outer_seed, inner_seed, monotonic
    )
    logger.info(
        f"Data created for anchor {train_size}. Sizes are {X_train.shape} (train), {X_valid.shape} (valid), and {X_test.shape} (test)."
    )
    ...
    logger.info(
        f"SVM fitted after {np.round(traintime, 2)}s. Now obtaining predictions."
    )
