...
class TracebackExtractor:
    ...
    def _get_experiment_campaigns_in_repo(repo):
        return TracebackExtractor._get_subfolders(repo)
    ...
    def get_results_for_all_configs(
        ...
        for repo in self.repos:
            for campaign in self._get_experiment_campaigns_in_repo(repo):
                ...
                for result_file in TracebackExtractor._get_result_files_in_folder(folder_with_seeds):
        ...
        # store non-repeating tracebacks in the format of set
        tracebacks = set()  
        for file in result_files:
            ...
            # filter the df column
            if 'traceback' in df.columns:
                tracebacks.update(df['traceback'].dropna().unique())
            else: 
                print("No Error! ")
        # DataFrame for output
        traceback_df = pd.DataFrame(list(tracebacks), columns=['traceback'])
