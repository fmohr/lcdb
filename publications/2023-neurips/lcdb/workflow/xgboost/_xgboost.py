...
# FIXME: if we use it adjust to make it work with new API
class EvalCallBack(TrainingCallback):
    def __init__(self, data_train, data_valid, data_test, encoder, learner):
        self.DM_train = DMatrix(data_train[0], label=encoder.transform(data_train[1]))
        self.y_train = data_train[1]
        self.DM_valid = DMatrix(data_valid[0], label=encoder.transform(data_valid[1]))
        self.y_valid = data_valid[1]
        self.DM_test = DMatrix(data_test[0], label=encoder.transform(data_test[1]))
    ...
    def after_iteration(self, model, epoch, evals_log):
        ...
        probs_pred_train = self.create_full_probs(probs_pred_train, n_classes=len(self.encoder.classes_))
        ...
        probs_pred_valid = self.create_full_probs(probs_pred_valid, n_classes=len(self.encoder.classes_))
        y_pred_valid = self.create_labels_from_probs(probs_pred_valid)

        probs_pred_test = model.predict(self.DM_test, strict_shape=True)
        probs_pred_test = self.create_full_probs(probs_pred_test, n_classes=len(self.encoder.classes_))
        ...
        self.metrics.append({"acc_train": acc_train, "acc_valid": acc_valid, "acc_test": acc_test})
        return False
...
class XGBoostWorkflow(PreprocessedWorkflow):
    ...
    def _fit(self, X, y, metadata):
        # FIXME: what we actually want to do is use the custom EvalCallBack from above to log metrics over the boosting iterations
        # however, we then require X_valid, y_valid, X_test, y_test to be passed to the fit method which is currently not supported
        # Also, the PrepocessedWorkflow does not support passing X_valid and y_valid at all
